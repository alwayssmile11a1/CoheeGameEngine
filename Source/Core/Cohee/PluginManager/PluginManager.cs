...
{
    /// <summary>
    /// Manage one or multiple <see cref="CoheePlugin"/> 
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public abstract class PluginManager<T> where T : CoheePlugin
    {
        ...
        /// </summary>
        public event EventHandler<CoheePluginEventArgs> PluginsRemoving = null;
        /// <summary>
        ...
        private void OnPluginsReady(IEnumerable<T> plugins)
        {
            if (this.PluginsReady != null)
                this.PluginsReady(null, new CoheePluginEventArgs(plugins));
        }
        private void OnPluginsRemoving(IEnumerable<T> oldPlugins)
        {
            if (this.PluginsRemoving != null)
                this.PluginsRemoving(null, new CoheePluginEventArgs(oldPlugins));
        }
        private void OnPluginsRemoved(IEnumerable<T> oldPlugins)
        {
            if (this.PluginsRemoved != null)
                this.PluginsRemoved(null, new CoheePluginEventArgs(oldPlugins));
